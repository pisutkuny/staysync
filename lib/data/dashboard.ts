...
export async function getDormName(organizationId?: number) {
    return unstable_cache(
        ...
        ['dashboard-dorm-name-v6', String(organizationId)],
        { revalidate: 3600 }
    )();
}
    ...
    return unstable_cache(
        async () => {
            try {
                ...
                console.log(`[Dashboard] getDashboardSummary processing for org: ${organizationId}`);
                let orgId = organizationId;
                if (!orgId || isNaN(orgId)) {
                    console.warn("[Dashboard] Invalid organizationId for summary, defaulting to 1");
                    orgId = 1;
                }
        ...
        ['dashboard-summary-v7', String(organizationId)],
        { revalidate: 60 }
    )();
}
...
export async function getRevenueChartData(organizationId: number): Promise<RevenueChartData[]> {
    return unstable_cache(
        ...
        ['dashboard-revenue-chart-v10', String(organizationId)],
        { revalidate: 300 }
    )();
}
...
export async function getOccupancyChartData(organizationId: number): Promise<OccupancyChartData[]> {
    return unstable_cache(
        ...
        ['dashboard-occupancy-v6', String(organizationId)],
        { revalidate: 60 }
    )();
}
...
export async function getRecentActivity(organizationId: number): Promise<ActivityItem[]> {
    return unstable_cache(
        ...
        ['dashboard-activity-v6', String(organizationId)],
        { revalidate: 30 }
    )();
}
...
export async function getTopSpenders(organizationId: number): Promise<TopSpenderItem[]> {
    return unstable_cache(
        ...
        ['dashboard-top-spenders-v6', String(organizationId)],
        { revalidate: 300 }
    )();
}
